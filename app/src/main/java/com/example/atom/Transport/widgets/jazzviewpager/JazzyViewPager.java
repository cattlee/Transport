...
在TypedArray后调用recycle主要是为了缓存。当recycle被调用后，这就说明这个对象从现在可以被重用了。TypedArray 内部持有部分数组，它们缓存在Resources类中的静态字段中，这样就不用每次使用前都需要分配内存。
	...
	private View wrapChild(View child) {
		...
		out.setLayoutParams(generateDefaultLayoutParams());//默认长和宽为WRAP_CONTENT
	...
	protected void animateTablet(View left, View right, float positionOffset) {
		if (mState != State.IDLE) {
			if (left != null) {
				...
				ViewHelper.setPivotX(left, left.getMeasuredWidth() / 2);//设置变换中心点包括Rotation和Scale
				ViewHelper.setPivotY(left, left.getMeasuredHeight() / 2);
				ViewHelper.setTranslationX(left, mTrans);//通过setTranslationX改变view的位置，是不改变view的LayoutParams的，也即不改变getLeft等view的信息
				ViewHelper.setRotationY(left, mRot);//轴心为穿过中心点的垂直轴
				logState(left, "Left");
			}
			if (right != null) {
   ...
   //图像绕中心点以Y轴为轴旋转时，宽度变化量的1/2
	...
	public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
		...
		//用于处理定时自动更新（）：mViewPager.setCurrentItem（）
